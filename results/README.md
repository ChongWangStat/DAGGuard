...
- `equal_sparsity_ablation_summary.csv` — diagnostic ablation holding graph size fixed. If BP retains K edges, the controls retain exactly K NOTEARS candidate edges by raw or unit-normalized coefficient magnitude. This is not a practical competing method; it isolates edge choice from the trivial effect of returning fewer edges.
...
- `unit_change_sensitivity.csv` — full-pipeline sensitivity after expressing two inventory variables in thousands; included to show explicitly that conditional BP scale invariance is not full-pipeline invariance.
